...
{
  "period_hours": 24,
  "generated_at": "2026-01-19T10:58:07.819442",
  "endpoints": [
    {
      "path": "/api/articles",
      "method": "GET",
      "requests_24h": 7,
      "avg_response_time_ms": 20.02,
      "max_response_time_ms": 42.49,
      "min_response_time_ms": 12.66,
      "success_rate": 100.0,
      "success_count": 7,
      "error_count": 0
    }
  ...
  "slowest_requests": [
    {
      "path": "/api/articles",
      "method": "GET",
      "response_time_ms": 42.49,
      "status_code": 200,
      "created_at": "2026-01-19 01:30:18"
    }
  ],
  "top_clients": [
    {"ip": "111.55.205.32", "requests": 24}
  ],
  "system": {
    "active_feeds": 7,
    "total_articles": 2822
...
- `generated_at`: 统计生成时间（ISO 8601 格式）
...
- `total_requests`: 总请求数
- `avg_response_time_ms`: 平均响应时间（毫秒）
- `success_rate`: 整体成功率（百分比）
- `success_count`: 成功请求数
- `server_errors`: 服务器错误数（5xx）
- `client_errors`: 客户端错误数（4xx）

**status_codes 数组**（状态码分布）:
- `code`: HTTP 状态码
- `count`: 出现次数

**slowest_requests 数组**（最慢的 10 个请求）:
- `path`: API 路径
- `method`: HTTP 方法
...
**top_clients 数组**（TOP 10 客户端）:
- `ip`: 客户端 IP 地址
- `requests`: 请求次数

**system 对象**（系统信息）:
- `active_feeds`: 活跃的 RSS 源数量
- `total_articles`: 文章总数
